...
from app.services.gemini import run_gemini_summary
...
async def summarize_clip(path: str = Query(...), db: Session = Depends(get_db)):
    transcript = transcribe_audio(path)
    summary = run_gemini_summary(transcript)
    summary_entry = Summary(video_path=path, summary=summary)
